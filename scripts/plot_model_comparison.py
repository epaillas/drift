...
Four curves per panel:
  ...
  eft/eft_lite  — pqg_eft_mu(mode="eft_lite")   [+ partial one-loop matter]
...
def _make_eft_callable(cosmo, ds_eft, gal_eft, p1loop, mode):
    """Return a (k, mu) -> P(k,mu) callable with p1loop captured in closure.

    A fresh loop_kwargs dict is created on every call so that the internal
    pop() inside pqg_eft_mu does not exhaust the precomputed value across
    multiple multipole projections.
    """
    def model(k, mu):
        lk = {"p1loop_precomputed": p1loop} if mode != "tree_only" else None
        return pqg_eft_mu(
            k, mu, z=Z, cosmo=cosmo,
            ds_params=ds_eft, gal_params=gal_eft,
            R=R, kernel=KERNEL, space=SPACE, ds_model=DS_MODEL,
            mode=mode, loop_kwargs=lk,
        )
    return model
...
def main():
    ...
    # Precompute one-loop matter power spectrum once (reused for all EFT calls)
    ...
    for row, q_idx in enumerate(QUANTILES):
        ...
        # --- EFT containers (all higher-order params = 0) ---
        ds_eft = DSSplitBinEFT(label=label_q, bq1=bq1)
        gal_eft = GalaxyEFTParams(b1=B1)

        all_callables = {
            "tree":          tree_model,
            "eft/tree_only": _make_eft_callable(cosmo, ds_eft, gal_eft, p1loop, "tree_only"),
            "eft/eft_lite":  _make_eft_callable(cosmo, ds_eft, gal_eft, p1loop, "eft_lite"),
            "eft/eft_full":  _make_eft_callable(cosmo, ds_eft, gal_eft, p1loop, "eft_full"),
        }
